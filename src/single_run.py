...
Runs exactly one prompting-strategy configuration (prompt_mode,
demonstration_mode/size, embedding_mode, retrieval_mode, thinking_mode) on a
70/30 train-test split (random_state=42) and writes one result CSV to
{results_path} using the standard result filename format (no fold suffix).
...
def main():
    ...
    print("Loading data...")
    train, test = util.load_data(config["data_path"] + "/def_train.csv")
    ...
    output_path = os.path.join(config["results_path"], result_filename(config))
