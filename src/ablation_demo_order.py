...
def run_config(cfg, train, test, lm, kb, annotations, split):
    ...
    if cfg["prompt_mode"] == "explicit":
        ...
        prompts = [pc.construct(text, order=cfg["order"]) for text in tqdm(test["description"])]
...
def main():
    ...
    base_results_path = cfg["results_path"] + "/ablations/demo_order"
    paths = {
    ...
    for order in DEMO_ORDERS:
        os.makedirs(os.path.join(base_results_path, order), exist_ok=True)
    ...
    for split in tqdm(range(N_SPLITS), desc="Folds"):
        ...
        for order in DEMO_ORDERS:
            cfg["order"]        = order
            cfg["results_path"] = os.path.join(base_results_path, order)
            ...
            print(f"  Classification for order: {order}")
            try:
